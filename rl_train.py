...
def parse_args():
    ...
    parser.add_argument('--vel_type',
                        ...
                        type=str)
    parser.add_argument('--mass_cons',
                        help='mass conservation loss weight',
                        default=0.,
                        type=float)
...
class RLConfig(Config):
    def __init__(self, _args):
        ...
        self.algo = "ppo"
        self.action_dim = 1 if args.env =="advection" else 2
...
if __name__ == '__main__':
    ...
    train_env_constructor = partial(get_environment,
                                    ...
                                    mass_cons_factor=config.mass_cons)
    ...
    trainer_kwargs = {
        ...
        'step_per_epoch': config.STEP_PER_EPOCH,  #TODO if not config.DEVICE == "cpu" else 5,  # to allow local debugging
    ...
    train_collector = Collector(policy, train_env, VectorReplayBuffer(config.REPLAY_BUFFER_SIZE, len(train_env)))
